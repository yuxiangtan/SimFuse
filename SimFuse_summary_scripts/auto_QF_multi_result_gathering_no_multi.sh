...
#1\get the folder list and use it into while loop to work for each folder (add no multi filtering)
...
then
  echo ""
    echo "Usage: auto_QF_multi_result_gathering_no_multi.sh split_num span_num"
    echo ""
    echo "split_num"
    echo "span_num"
...
if [ -s all_sum_file_no_multi.txt ] 
then
	rm -rf all_sum_file_no_multi.txt
fi
...
do
    ...
    then
            ...
            #cat $whole_sum_file >> all_sum_file_no_multi.txt
            #because I have header in the output now
            #while read WSF;
            #do
            #    multi_num=`echo $WSF | cut -f17 -d" "`
            #    if [ $multi_num == 1 ];
            #    then
            #        echo $WSF >> all_sum_file_no_multi.txt
            #    fi
            #done < $whole_sum_file
            awk -F "\t" '{if($17 == 1){print}}' $whole_sum_file >> all_sum_file_no_multi.txt
    fi
...
then
    ...
    #grep $chr1 all_sum_file_no_multi.txt | grep $chr2 |grep $BP1_use | grep $BP2_use >> TP_fusion_list.txt
    #
    #done < "../../coverage_on_exons.txt_"$split_num"_"$span_num"_ref_bp.txt"
    python $fusion_simulator_path"QF_result_summary_on_simulation_not_considerID.py" "all_sum_file_no_multi.txt" "../../coverage_on_exons.txt_"$split_num"_"$span_num"_ref_bp.txt" $split_num $span_num "../../QF_on_"$split_num"_"$span_num"_ref_bp_summary_no_multi"
    ...
    #        #grep $ENS1 all_sum_file_no_multi.txt | grep $ENS2 >> "all_fusion_found_list_"$i"_"$j".txt"
    #        grep $chr1 all_sum_file_no_multi.txt | grep $chr2 |grep $BP1_use | grep $BP2_use >> "TP_fusion_list_"$i"_"$j"_bp.txt"
    #        grep $chr1 all_sum_file_no_multi.txt | grep $chr2 |grep $BP1_use | grep $BP2_use | cut -f5,10 | sort -u >> "TP_fusion_list_"$i"_"$j".txt"
    #        done < "../../coverage_on_exons.txt_"$split_num"_"$span_num"_ref_bp_"$i"_"$j".txt"
    #    done
    #done

else
    echo "">all_sum_file_no_multi.txt
    python $fusion_simulator_path"QF_result_summary_on_simulation_not_considerID.py" "all_sum_file_no_multi.txt" "../../coverage_on_exons.txt_"$split_num"_"$span_num"_ref_bp.txt" $split_num $span_num "../../QF_on_"$split_num"_"$span_num"_ref_bp_summary_no_multi"
    echo "all_sum_file_no_multi.txt is empty, which means no fusions found."
    exit 1
fi
...
#fusion_all_found=`cut -f5,10 all_sum_file_no_multi.txt |sort -u |wc -l | cut -f1 -d" "`
#fusion_TP=`wc -l TP_fusion_ID_pairlist.txt | cut -f1 -d" "`
#split_all_found=`awk '{print $(11)}' all_sum_file_no_multi.txt | xargs |sed 's/ /+/g' | bc`
#split_TP=`awk '{print $(11)}' TP_fusion_list.txt | xargs |sed 's/ /+/g' | bc`
#span_all_found=`awk '{print $(12)}' all_sum_file_no_multi.txt | xargs |sed 's/ /+/g' | bc`
