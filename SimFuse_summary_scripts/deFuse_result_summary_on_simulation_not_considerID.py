...
#compare defuse result to simulated fusion bp to check TP and FP.
#From defuse, a few columns needed: chr(25,26) , bp(38,39), ID(21,22), split_num (3), span_num (57).

#Things to consider:
#1\same simulation can have multiple output in defuse. (sort 2ID, left is the small one and right is the bigger one, then use a list to eliminate duplicates)
...
#example: simulated: chr10_bp92638891_ENSG00000148688_chr9_bp100112691_ENSG00000197816; deFuse: chr10_92638891_ENSG00000148688_chr9_100112691_ENSG00000255036
...
if __name__ == "__main__":
	...
	if len(sys.argv) < 6:
		...
		print "Usage: python deFuse_result_summary_on_simulation.py defuse_result_file fusion_bp_file simulated_split_num simulated_span_num summary_defuse_out"
		print "Example:"
		print "data_defuse=defuse_50_50_ref_merged/results.filtered.tsv"
		print "data_sim=coverage_on_exons.txt_50_50_ref_bp.txt"
		print "split_num=50"
		print "span_num=50"
		print "sum_out=defuse_on_50_50_ref_bp_summary"
		sys.exit(1)
		
	
	#this version is with no mutation_rate_file (not used anywhere)
	data_defuse = sys.argv[1]
	data_sim = sys.argv[2]
	...
	#open the files
	h_def=open(data_defuse,"r")
	h_sim=open(data_sim,"r")
	h_def_dedup_out=open(data_defuse+"_dedup.txt","w")
	...
	print "Generating deduplicated defuse result"
	#deduplicate for defuse result and extract informations needed:
	for row_num in range(len(def_matrix)):
		#chr(25,26) , bp(38,39), ID(21,22), split_num (3), span_num (57)
		chr_end1=def_matrix[row_num][24]
		chr_end2=def_matrix[row_num][25]
		bp_end1=def_matrix[row_num][37]
		bp_end2=def_matrix[row_num][38]
		ID_end1=def_matrix[row_num][20]
		ID_end2=def_matrix[row_num][21]
		split_num=def_matrix[row_num][2]
		span_num=def_matrix[row_num][56]
		
		#reorder then fusion pairs to make ID1 smaller than ID2
		if chr_end1<chr_end2:
			fusion_pair="chr"+chr_end1+"_"+bp_end1+"_"+ID_end1+"_"+"chr"+chr_end2+"_"+bp_end2+"_"+ID_end2
		else: 	
			if chr_end1>chr_end2:
				fusion_pair="chr"+chr_end2+"_"+bp_end2+"_"+ID_end2+"_"+"chr"+chr_end1+"_"+bp_end1+"_"+ID_end1
			else:
				if bp_end1<bp_end2:
					fusion_pair="chr"+chr_end1+"_"+bp_end1+"_"+ID_end1+"_"+"chr"+chr_end2+"_"+bp_end2+"_"+ID_end2
				else:
					if bp_end1>bp_end2:
						...
						print "Warning: there is a fusion pair from deFuse has both breakpoint the same"
						continue
	...
	h_def_dedup=open(data_defuse+"_dedup.txt","r")
